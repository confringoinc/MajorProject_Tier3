...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIControl : MonoBehaviour, IUpdate
    {
		...
		public int infront_or_behind = 0;
		...
		public void Spawn(List<GameObject> cars)
		{
			...
			initial_position = m_CarController.transform.position;
			initial_rotation = m_CarController.transform.rotation;
			...
			while(!spawn_check && escape_cnt < 500)
			{
				...
				if (forward) 
				{
					...
					m_CarController.setMaxSpeed (40 + 10 * Random.Range (0.0f, 1.0f));
					//m_CarController.setMaxSpeed (40);
					//int infront_or_behind = Random.Range (0, 2);
					if (infront_or_behind == 1) 
					{
						waypoint_offset = Random.Range (3, 5);
						m_CarController.setMaxSpeed (40 + 10 * Random.Range (-1.0f, 0.0f));
						//m_CarController.setMaxSpeed (40);
					}
					infront_or_behind = (infront_or_behind + 1) % 2;
				} 
				else 
				{
					waypoint_offset = Random.Range (3, 5);
				...
				Vector3 start_offset = waypoints [compare_start].right * 1*direction;
				if (lane == 1)
				{
					start_offset = waypoints [compare_start].right * 3*direction;
				} 
				else if(lane==2) 
				{
					start_offset = waypoints [compare_start].right * 5*direction;
				}
			...
			if (escape_cnt < 500) 
			{
				...
				// Debug.LogError(m_Rigidbody.name+" : Rigid Body Velocity:- " + m_Rigidbody.velocity);
				
				// m_Rigidbody.position += new Vector3(m_CarController.CurrentSteerAngle*40f, 0f, 0f);
		...
		public void removeCar(){
			...
			m_CarController.transform.rotation = initial_rotation;
        ...
        public void performUpdate(float time){}

        public void performFixedUpdate (float time)
        {	
			if (!maincar) 
			{
				// Debug.LogError("AICar Fixed Update:- " + m_Rigidbody.position + " Car Steering Angle:- " + m_CarController.CurrentSteerAngle);
				// Quaternion deltaRotation = Quaternion.Euler(new Vector3(m_CarController.CurrentSteerAngle, 0, 0)*time);
				// this.m_Rigidbody.MoveRotation(this.m_Rigidbody.rotation*deltaRotation);
				
				// this.m_Rigidbody.transform.Translate(m_Rigidbody.transform.forward*time);
				...
				if (m_Target == null || !m_Driving) {
					...
					// float toSteer = steer * 25;
					

					// Vector3 m_EulerAngleVelocity = new Vector3(0, toSteer, 0);
					// Quaternion deltaRotation = Quaternion.Euler(m_EulerAngleVelocity*40f*time);
					// this.m_Rigidbody.MoveRotation(m_Rigidbody.rotation*deltaRotation);

					// this.m_Rigidbody.MovePosition(m_Rigidbody.transform.position + (this.transform.forward*40f*time));
					
					// if(this.m_Rigidbody.name.Equals("CarAI1")){
					// 	Debug.LogError("Steering Angle:- " + m_CarController.m_WheelColliders[0].steerAngle);
					// }

					// ####### feed input to the car controller. ########
					float initVel = m_Rigidbody.velocity.magnitude;
					// m_CarController.steerWheel(steer, accel, accel);
					
					m_CarController.Move(steer, accel, accel, 0f);

					// if(m_Rigidbody.name.Equals("CarAI1")){
					// 	Debug.LogError("init Vel:- " + initVel + " | MainVel:- " + m_Rigidbody.velocity.magnitude);
					// }
			...
			else if(maincar)
			{
				...
				dist_eval += (time * get_speed/2.23693629f)/1609.34f;
				time_eval += time;
