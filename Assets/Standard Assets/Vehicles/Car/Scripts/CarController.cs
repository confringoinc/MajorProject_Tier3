...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarController : MonoBehaviour, IUpdate
    {
        ...
        //     Debug.LogError("Car: " + this.gameObject.name);
        //     accel = Mathf.Clamp(accel, -1, 1);
        //     steer = Mathf.Clamp(steer, -1, 1) * m_MaximumSteerAngle;
        //     brake = Mathf.Clamp(brake, 0, 1) * m_MaxHandbrakeTorque; //[0,1] Because we don't want to go in negative direction (reverse) by applying brake;
        //     float thurstTorque = 0;
            
        //     if(CurrentSpeed < m_Topspeed)
        //         thurstTorque = accel * 200;
            
        //     for(int i=0; i<4; i++){
        //         m_WheelColliders[i].motorTorque = thurstTorque;
                
        //         if(i<2)
        //             m_WheelColliders[i].steerAngle = steer;
        //         else
        //             m_WheelColliders[i].brakeTorque = brake;
        //         Quaternion quat;
        //         Vector3 position;
        ...
        public void performUpdate(float time){
            if(main_car){
                Vector3 localTarget = m_Rigidbody.gameObject.transform.InverseTransformPoint(target); //Vehicle's RB is origin wrt LocalTarget
                float distanceToTarget = Vector3.Distance(target, m_Rigidbody.gameObject.transform.position);

                float targetAngle = Mathf.Atan2(localTarget.x, localTarget.z) * Mathf.Rad2Deg;
                float steer = Mathf.Clamp(targetAngle * steeringSensitivity, -1, 1) * Mathf.Sign(CurrentSpeed);
                float accel = 0.25f;
                //float brake = 0;

                //Go(accel, steer, brake);
                Move(steer, accel, accel, 0f);
        ...
        public void performFixedUpdate(float time)
        {
			if (main_car) 
			{
				...
				if (averageSpeed.Count >= time_steps) {
					...
					AccelerationT = (averaged_speed - lastSpeed) / (time_steps*time);
				...
				if (averageAcc.Count >= 5) 
				{
					...
					Jerk = (averaged_acc - lastAcc) / ((5*time_steps) * time);
        ...
        {
            // if(this.name.Equals("CarAI1"))
            // {
            //     float initVel = m_Rigidbody.velocity.magnitude;
            //     Debug.LogError("InitVel:- " + initVel);
            // }
            ...
            // Debug.LogError("Initial Steering Angle:- " + m_WheelColliders[0].steerAngle);
            SteerHelper ();

            // Debug.LogError("After SH Steering Angle:- " + m_WheelColliders[0].steerAngle);
            ApplyDrive (accel, footbrake);
            // if(this.name.Equals("CarAI1")){
            //     Debug.LogError("Steering Angle:- " + steering + " | Velocity: " + m_Rigidbody.velocity.magnitude);
            // }

            // Debug.LogError("After AD Steering Angle:- " + m_WheelColliders[0].steerAngle);
            //CapSpeed ();
            ...
            // if(this.name.Equals("CarAI1")){
            //     Debug.LogError("Main Vel: " + m_Rigidbody.velocity.magnitude);
            // }
        }
        
        // public void steerWheel(float steering, float accel, float footbrake){
        //     steering = Mathf.Clamp (steering, -1, 1);
        //     //Set the steer on the front wheels.
        //     //Assuming that wheels 0 and 1 are the front wheels.
        //     for (int i = 0; i < 4; i++) {
